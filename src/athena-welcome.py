...
class Main(Gtk.Window):
    ...
    def on_channel_clicked(self, widget):
        t = threading.Thread(target=self.channel_update)
        t.daemon = True
        t.start()
    ...
    def on_gp_clicked(self, widget):
        t = threading.Thread(target=self.run_app, args=(["gparted"],))
        t.daemon = True
        t.start()
    ...
    def on_buttontools_clicked(self, widget):
        t = threading.Thread(target=self.run_app, args=(["python", "./tool-recipe.py"],))
        t.daemon = True
        t.start()
    ...
    def on_buttonroletools_clicked(self, widget):
        t = threading.Thread(target=self.run_app, args=(["python", "./role-viewer.py"],))
        t.daemon = True
        t.start()

    def on_buttonupdate_clicked(self, widget):
        t = threading.Thread(target=self.run_app, args=(["shell-rocket", "sudo nix-channel --update; sudo nixos-rebuild switch"],))
        t.daemon = True
        t.start()

    def on_buttonhv_clicked(self, widget):
        t = threading.Thread(target=self.run_app, args=(["python", "./hacking-variables.py"],))
        t.daemon = True
        t.start()
    ...
    def on_launch_clicked(self, widget, event, link):
        if os.path.isfile("archlinux-tweak-tool"):
            t = threading.Thread(target=self.run_app,
                                 args=("archlinux-tweak-tool",))
            t.daemon = True
            t.start()
        else:
...
Do you want to install it?")
    ...
    def internet_notifier(self):
        ...
        while(True):
            if not self.is_connected():
                dis = 1
                GLib.idle_add(self.bntchannels.set_sensitive, False)
                ...
                if bb == 0 and dis == 1:
                    GLib.idle_add(self.bntchannels.set_sensitive, True)
    ...
    def channel_update(self):
        GLib.idle_add(self.cc.set_markup, "<span foreground='orange'><b><i>Updating Nix channels</i></b> \nThis may take some time, please wait...</span>")  # noqa
        GLib.idle_add(self.bntchannels.set_sensitive, False)
        subprocess.run(["pkexec", "nix-channel", "--update"], shell=False)
        print("FINISHED!!!")
